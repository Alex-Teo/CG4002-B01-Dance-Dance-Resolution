...
//	DTYPE weights_4[LAYER4_H][LAYER4_W];
	DTYPE bias_0[LAYER0_W];
	DTYPE bias_1[LAYER1_W];
...
//	DTYPE buffer_4[LAYER4_H];
	...
	node<DTYPE, LAYER0_H, 0> node_1;
//	node<DTYPE, POS_LAYER0_H, 0> node_2;
	while (true) {
		AXI_L in, out;
		if (staged == 0) {
...
//			}
			loadbias0:for (int i = 0; i < LAYER0_W; i++) {
				S_AXIS >> in;
				bias_0[i] = in.data;
			}
			loadbias1:for (int i = 0; i < LAYER1_W; i++) {
				S_AXIS >> in;
				bias_1[i] = in.data;
			}
